...
func Test_jsonToGo(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Array Mixed     - Uppercase", wantErr: false, args: args{typename: "AutoGenerated", jsonString: []byte("{\"MyArray\":[1.0,2,\"strings\"]}")}, want: "type AutoGenerated struct { MyArray []interface{} `json:\"MyArray\"`}"},
		{name: "Array Obj       - Uppercase", wantErr: false, args: args{typename: "AutoGenerated", jsonString: []byte("{\"MyArray\":[{\"myDec\":1.0},{\"myInt\":2}]}")}, want: "type AutoGenerated struct { MyArray []interface{} `json:\"MyArray\"`}"},
	}
